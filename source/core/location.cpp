...
int CPlateLocation::plateSobelLocate(Mat src, vector<CPlate> &candPlates, int index)
{
...
int CPlateLocation::sobelSecSearch(Mat &bound, Point2f refpoint, vector<RotatedRect> &outRects)
{
	Mat bound_threshold;
...
int CPlateLocation::sobelOperT(const Mat &in, Mat &out, int blurSize, int morphW, int morphH)
{
	Mat mat_blur;
	mat_blur = in.clone();
...
int CPlateLocation::sobelSecSearchPart(Mat &bound, Point2f refpoint, vector<RotatedRect> &outRects)
{
	Mat bound_threshold;
...
int CPlateLocation::sobelFrtSearch(const Mat &src, vector<Rect_<float>> &outRects)
{
	Mat src_threshold;
...
int CPlateLocation::sobelOper(const Mat &in, Mat &out, int blurSize, int morphW, int morphH)
{
	Mat mat_blur;
	mat_blur = in.clone();
	GaussianBlur(in, mat_blur, Size(blurSize, blurSize), 0, 0, BORDER_DEFAULT);
...
bool CPlateLocation::VerifySizes(RotatedRect mr)   //大小匹配
{
	float error = m_error;
...
//找到匹配的颜色
int CPlateLocation::colorSearch(const Mat &src, const Color r, Mat &out, vector<RotatedRect> &outRects)
{
	Mat match_grey;

	// width is important to the final results;
	const int morph_width = 10;
	const int morph_height = 2;
	imshow("match_grey", src);
	waitKey(1);
	ColorMatch(src, match_grey, r, false); //获得匹配后的二进制图像
	...
	src_threshold.copyTo(out);  //更快
	//out = src_threshold.clone();
...
bool CPlateLocation::rotation(Mat &in, Mat &out, const Size rect_size,
...
bool CPlateLocation::isdeflection(const Mat &in, const double angle,double &slope)
{ /*imshow("in",in);
					waitKey(0);*/
	if (0) {
		imshow("in", in);
...
void CPlateLocation::affine(const Mat &in, Mat &out, const double slope)
{
	// imshow("in", in);
	// waitKey(0);
...
int CPlateLocation::deskew(const Mat &src, const Mat &src_b, vector<RotatedRect> &inRects,
	vector<CPlate> &outPlates, bool useDeteleArea, Color color)
{
	...
	for (size_t i = 0; i < inRects.size(); i++)
	{
		...
		if (roi_angle - m_angle < 0 && roi_angle + m_angle > 0)   //m_angle 默认60度
		{
			Rect_<float> safeBoundRect;
			bool isFormRect = calcSafeRect(roi_rect, src, safeBoundRect);  //避免出现异常roi
			if (!isFormRect) continue;

			Mat bound_mat = src(safeBoundRect); //原图截取返回
			Mat bound_mat_b = src_b(safeBoundRect);  //处理后的图片截取返回
...
int CPlateLocation::plateColorLocate(Mat src, vector<CPlate> &candPlates, int index )
{
	vector<RotatedRect> rects_color_blue;
	rects_color_blue.reserve(64);          //最多可存64个RotatedRect
	vector<RotatedRect> rects_color_yellow;
	rects_color_yellow.reserve(64);

	vector<CPlate> plates_blue;
	plates_blue.reserve(64);    //最多可存64个CPlate
...
//车牌定位
void CPlateLocation::plateLocate(Mat src, vector<Mat> &resultVec, int index )
{
